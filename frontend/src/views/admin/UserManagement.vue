    ...
    <main class="flex-1 overflow-y-auto bg-gradient-to-br from-blue-50 to-blue-100 p-6">
      <AppPageContainer variant="gradient" padding="lg" max-width="2xl">
        ...
        <div class="grid grid-cols-1 md:grid-cols-4 gap-6 mb-8">
          <AppCard class="text-center"><div class="text-3xl font-bold text-blue-600">{{ stats.userCount }}</div><div class="text-gray-500 mt-2">普通用户数</div></AppCard>
          <AppCard class="text-center"><div class="text-3xl font-bold text-blue-600">{{ stats.organizerCount }}</div><div class="text-gray-500 mt-2">组织者</div></AppCard>
          <AppCard class="text-center"><div class="text-3xl font-bold text-blue-600">{{ stats.adminCount }}</div><div class="text-gray-500 mt-2">管理员</div></AppCard>
          <AppCard class="text-center"><div class="text-3xl font-bold text-orange-500">{{ stats.pendingOrganizerCount }}</div><div class="text-gray-500 mt-2">待审批组织者</div></AppCard>
        </div>
              ...
              <table class="min-w-full divide-y divide-gray-200">
                ...
                <tbody class="divide-y divide-gray-100">
                  <tr v-for="org in organizers" :key="org.organizer_id">
                    ...
                    <td class="px-6 py-4"><button @click="openOrgDetail(org)" class="px-3 py-1 bg-blue-100 text-blue-700 rounded text-sm">详情</button></td>
                  </tr>
                </tbody>
              </table>
            </div>
        ...
        <AppDialog v-model:open="adminModalVisible" title="新增管理员" confirm-text="添加" cancel-text="取消" @confirm="addAdmin">
          <div class="space-y-3"><input type="text" v-model="newAdmin.username" placeholder="用户名" class="w-full border rounded px-3 py-2"><input type="email" v-model="newAdmin.email" placeholder="邮箱" class="w-full border rounded px-3 py-2"><input type="password" v-model="newAdmin.password" placeholder="初始密码" class="w-full border rounded px-3 py-2"></div>
        </AppDialog>
      </AppPageContainer>
    </main>
  </div>
...
const confirmRejectOrganizer = async () => {
  ...
  try {
    await request.put(`/admin/organizers/${currentOrg.value.organizer_id}/review`, { action: 'reject', reject_reason: rejectReason.value })
    alert('已拒绝')
    orgModalVisible.value = false
    fetchOrganizers()
  } catch (e) {
...
const addAdmin = async () => {
  ...
  try {
    await request.post('/admin/admins', newAdmin)
    alert('管理员添加成功')
