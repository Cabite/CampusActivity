      ...
      <AppPageContainer variant="gradient" padding="lg" max-width="2xl">
        ...
        <AppCard :loading="loading">
          ...
          <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8">
            <div class="bg-blue-50 rounded-xl p-4 text-center"><div class="text-3xl font-bold text-blue-600">{{ stats.totalRegistered }}</div><div class="text-gray-600">报名人数</div></div>
            <div class="bg-green-50 rounded-xl p-4 text-center"><div class="text-3xl font-bold text-green-600">{{ stats.checkedInCount }}</div><div class="text-gray-600">签到人数</div></div>
            <div class="bg-purple-50 rounded-xl p-4 text-center"><div class="text-3xl font-bold text-purple-600">{{ stats.checkinRate }}%</div><div class="text-gray-600">签到率</div></div>
          </div>
          <div class="grid grid-cols-1 lg:grid-cols-2 gap-8">
            <div class="border rounded-xl p-4"><h3 class="text-md font-semibold text-center mb-3">性别分布</h3><div v-for="(value, key) in stats.byGender" :key="key" class="mb-2"><div class="flex justify-between text-sm"><span>{{ key }}</span><span>{{ value }}%</span></div><div class="w-full bg-gray-200 rounded-full h-2"><div class="bg-blue-600 h-2 rounded-full" :style="{ width: value + '%' }"></div></div></div></div>
            <div class="border rounded-xl p-4"><h3 class="text-md font-semibold text-center mb-3">专业分布</h3><div v-for="(value, key) in stats.byMajor" :key="key" class="mb-2"><div class="flex justify-between text-sm"><span>{{ key }}</span><span>{{ value }}%</span></div><div class="w-full bg-gray-200 rounded-full h-2"><div class="bg-green-600 h-2 rounded-full" :style="{ width: value + '%' }"></div></div></div></div>
            <div class="border rounded-xl p-4"><h3 class="text-md font-semibold text-center mb-3">年级分布</h3><div v-for="(value, key) in stats.byGrade" :key="key" class="mb-2"><div class="flex justify-between text-sm"><span>{{ key }}</span><span>{{ value }}%</span></div><div class="w-full bg-gray-200 rounded-full h-2"><div class="bg-yellow-600 h-2 rounded-full" :style="{ width: value + '%' }"></div></div></div></div>
            <div class="border rounded-xl p-4"><h3 class="text-md font-semibold text-center mb-3">学院分布</h3><div v-for="(value, key) in stats.byCollege" :key="key" class="mb-2"><div class="flex justify-between text-sm"><span>{{ key }}</span><span>{{ value }}%</span></div><div class="w-full bg-gray-200 rounded-full h-2"><div class="bg-red-600 h-2 rounded-full" :style="{ width: value + '%' }"></div></div></div></div>
          </div>
        </AppCard>
      </AppPageContainer>
    </main>
